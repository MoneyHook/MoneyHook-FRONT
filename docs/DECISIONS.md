...
| APIの成功レスポンスを初期表示・障害時のフォールバック用に永続化し、画面のmount時に再取得する | 再訪時の表示を補助しつつ、APIを正本に保つため。ユーザー変更・ログアウト時は保存データを破棄する |
| デフォルト支払い方法はユーザーに紐づく端末内設定として保存する                               | 新規取引の初期選択を保持するため。APIには同期せず、対象の支払い方法がなくなった場合は解除する   |
