/**
 * Generated by orval 🍺
 * Do not edit manually.
 * MoneyHooks API
 * Go/Echo implementation as audited from `moneyHook_api` on 2026-08-26.
 *
 * This is a code-first description. Legacy transaction writes validate required fields and
 * values before persistence. Other legacy write endpoints still have gaps in request validation.
 *
 * Amount convention: write requests send a non-negative magnitude and `*_sign`; the server
 * stores their product. Several aggregate responses therefore expose expenses as negative values.
 *
 * すべての業務APIは、Googleログインで発行されたFirebase IDトークンをBearer認証として
 * 検証する。GET / だけはプロセスヘルスチェックとして公開する。
 *
 * OpenAPI spec version: 0.2.0-v1
 */
import type { DateString } from './dateString';
import type { Identifier } from './identifier';
import type { NullableIdentifier } from './nullableIdentifier';
import type { V1NullableTime } from './v1NullableTime';
import type { V1TransactionCreateInputSign } from './v1TransactionCreateInputSign';

/**
 * Exactly one of `sub_category_id` or `sub_category_name` must be supplied.
 */
export interface V1TransactionCreateInput {
  transaction_date: DateString;
  transaction_time?: V1NullableTime | null;
  /**
     * @minLength 1
     * @maxLength 32
     */
  transaction_name: string;
  /**
     * @minimum 1
     * @maximum 9999999
     */
  amount: number;
  sign: V1TransactionCreateInputSign;
  category_id: Identifier;
  /** Existing subcategory ID. Mutually exclusive with `sub_category_name`. */
  sub_category_id?: Identifier;
  /**
     * Trimmed name to find, expose, or create. Mutually exclusive with `sub_category_id`.
     * @maxLength 16
     */
  sub_category_name?: string;
  fixed_flg: boolean;
  payment_id?: NullableIdentifier | null;
}
