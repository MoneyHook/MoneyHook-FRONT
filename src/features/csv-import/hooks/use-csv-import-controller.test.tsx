...
describe('CSV import controller', () => {
  it('parses, maps and edits rows, then preserves them after a failed import and completes on retry', async () => {
    ...
    server.use(
      http.post(
        'http://api.test/api/transaction/addTransactionList',
        async ({ request }) => {
  ...
  it('checks each import month once, flags matches, and permits import after a check failure', async () => {
    ...
    server.use(
      http.get(
        'http://api.test/api/transaction/getTimelineData',
        ({ request }) => {
    ...
    expect(result.current.duplicateCandidates.get(0)).toEqual([
      expect.objectContaining({ transaction_id: 'existing' }),
    ])
