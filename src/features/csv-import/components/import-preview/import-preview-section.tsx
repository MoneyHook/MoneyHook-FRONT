...
export function ImportPreviewSection({ categories, dispatch, errors, filter, filteredRows, importing, onFilterChange, onSubmit, payments, previewRows, selected, selectedErrors }: {
  ...
  return <section className="grid gap-5 border-t py-8">
    <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="text-lg font-semibold">インポート内容を確認</h2><p className="text-sm text-muted-foreground">{selected} / {previewRows.length}件をインポート予定 ・ エラー {errors}件</p></div><div className="flex flex-wrap gap-2">{([['all', 'すべて'], ['selected', '対象'], ['excluded', '対象外'], ['error', 'エラー']] as const).map(([value, label]) => <Button key={value} onClick={() => onFilterChange(value)} size="sm" variant={filter === value ? 'default' : 'outline'}>{label}</Button>)}</div></div>
    ...
    <div className="flex justify-end"><Button disabled={!selected || selectedErrors > 0 || importing} onClick={onSubmit}>{importing ? <><LoaderCircle className="animate-spin" />登録しています...</> : <><Upload />{selected}件をインポート</>}</Button></div>
  </section>
}
