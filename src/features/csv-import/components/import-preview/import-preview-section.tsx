...
export function ImportPreviewSection({
  ...
  return (
    <section className="grid gap-5 border-t py-8">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="text-lg font-semibold">インポート内容を確認</h2>
          <p className="text-sm text-muted-foreground">
            {selected} / {previewRows.length}件をインポート予定 ・ エラー{' '}
            {errors}件 ・ 重複候補 {duplicateCount}件
          </p>
        </div>
      ...
      {failedDuplicateCheckMonths.length ? (
        <Alert className="border-warning/40 bg-warning/10 text-warning">
          <AlertCircle aria-hidden="true" />
          <AlertTitle>一部の重複候補を確認できませんでした</AlertTitle>
          <AlertDescription className="text-warning/90">
            {failedDuplicateCheckMonths.join('、')}
            の既存取引を取得できませんでした。未確認の状態でもインポートできます。
          </AlertDescription>
        </Alert>
      ) : null}
      <div className="flex flex-wrap gap-2">
        ...
        <BulkTransactionEditDialog
          categories={categories}
          onApply={(rowIds, categoryId, subcategoryId, paymentId) =>
            dispatch({
      ...
      <div className="flex justify-end">
        <Button
          disabled={
            !selected || selectedErrors > 0 || isCheckingDuplicates || importing
          }
          onClick={onSubmit}
        >
          {importing ? (
