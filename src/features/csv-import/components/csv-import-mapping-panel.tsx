...
export function CsvImportMappingPanel({
  dateFormat,
  encoding,
  file,
  headerRowIndex,
    ...
    <aside>
      <Card className="grid content-start gap-4 p-5">
        <div className="flex items-start justify-between gap-3">
          <div>
            <h2 className="font-semibold">列の指定</h2>
            <p className="mt-1 text-sm text-muted-foreground">
              {file?.name}・{parsedEncoding?.toUpperCase()}として解析
            </p>
          </div>
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button
                aria-label="解析オプションを開く"
                size="icon"
                variant="ghost"
              >
                <Ellipsis />
            ...
            <DropdownMenuContent align="end" className="w-72 p-3">
              <DropdownMenuLabel>解析オプション</DropdownMenuLabel>
              <DropdownMenuSeparator />
              <div className="grid gap-3 px-1 py-2">
                <Field label="文字コード">
                  <SelectField
                    value={encoding}
                    onValueChange={(value) =>
                      onEncodingChange(value as Encoding)
                    }
                  >
                ...
                <Field label="日付形式">
                  ...
                  >
                    ...
                    <option value="japanese">YYYY年M月D日</option>
                  </SelectField>
                </Field>
              </div>
            </DropdownMenuContent>
        ...
        <Field label="ヘッダー行">
          <SelectField
            value={headerRowIndex === null ? 'none' : String(headerRowIndex)}
            onValueChange={(value) =>
              onHeaderRowChange(value === 'none' ? null : Number(value))
            }
          >
            <option value="none">ヘッダーなし</option>
        ...
        {(['date', 'name', 'amount'] as const).map((field) => (
          <Field
            key={field}
            label={{ date: '日付', name: '取引名', amount: '金額' }[field]}
          >
            <SelectField
              value={mapping[field] === null ? '' : String(mapping[field])}
              onValueChange={(value) =>
                onMappingChange(field, value === '' ? null : Number(value))
              }
            >
