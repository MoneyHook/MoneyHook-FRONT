...
export function FixedSummaryCard({
  ...
  return (
    <DashboardCard>
      <h2 className="text-sm leading-none font-semibold sm:text-lg sm:leading-normal">
        固定費サマリー
      </h2>
      <div className="mt-2 grid grid-cols-[minmax(0,0.92fr)_minmax(0,1.08fr)] items-center gap-2 sm:mt-5 sm:grid-cols-[minmax(14rem,0.8fr)_1.2fr] sm:gap-6">
        <div className="rounded-xl bg-success/8 p-2 sm:p-4">
          <div className="flex items-center gap-2 sm:gap-4">
            ...
            <div>
              ...
              <p className="text-lg leading-none font-semibold tabular-nums sm:mt-1 sm:text-2xl sm:leading-normal">
                {formatCurrency(data.fixedMonthlyAmount)}
              </p>
            </div>
          </div>
          <p className="mt-1.5 text-[0.5625rem] whitespace-nowrap text-muted-foreground sm:mt-4 sm:text-sm">
            年間換算{' '}
        ...
        <div>
          ...
          <p className="text-xl leading-none font-semibold tabular-nums sm:mt-1 sm:text-2xl sm:leading-normal">
          ...
          <div className="mt-1 h-1.5 overflow-hidden rounded-full bg-muted sm:mt-3 sm:h-2">
            <span
              ...
              style={{
                width: `${Math.min(data.fixedTotalExpenseRatio, 100)}%`,
              }}
            />
          </div>
          <div className="mt-1 border-t pt-1 text-right sm:mt-5 sm:pt-3">
            <Button
              asChild
              className="h-6 px-0 text-[0.625rem] leading-none text-primary sm:h-auto sm:text-sm sm:leading-normal"
              variant="link"
            >
