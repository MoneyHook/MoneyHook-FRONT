...
function CategoryRow({ category }: { category: CategorySummary }) {
  ...
  return (
    <li className="grid grid-cols-[auto_1fr_auto] items-center gap-x-3">
      ...
      <span className="truncate text-xs leading-none font-medium sm:text-sm sm:leading-normal">
        {category.name}
      </span>
      <span className="text-xs leading-none font-semibold tabular-nums sm:text-sm sm:leading-normal">
      ...
      <span
        className={cn(
          'text-[0.625rem] leading-none font-medium tabular-nums sm:mt-0.5 sm:text-xs sm:leading-normal',
          differenceClass,
        )}
      >
  ...
  return (
    <DashboardCard className="flex min-h-full flex-col">
      <h2 className="text-sm font-semibold sm:text-lg">
        カテゴリ別支出{' '}
        <span className="text-[0.625rem] sm:text-xs">（上位5件）</span>
      </h2>
      ...
      <Button
        asChild
        className="mt-1 h-6 self-center text-xs text-primary sm:mt-5 sm:h-9 sm:text-sm"
        variant="link"
      >
