    ...
    <ul className="divide-y rounded-xl border">
      {rules.map((rule) => {
        ...
        const paymentName = rule.payment_id
          ? payments.find((payment) => payment.payment_id === rule.payment_id)
              ?.payment_name
          : null
        ...
        return (
          <li
            ...
            key={rule.monthly_transaction_id}
          >
            <span
              className={cn(
            ...
            <div className="min-w-0 flex-1">
              <div className="flex flex-wrap items-baseline gap-x-2 gap-y-0.5">
                ...
                >
                  {isIncome ? '+' : '−'}¥
                  {rule.monthly_transaction_amount.toLocaleString('ja-JP')}
                </span>
              </div>
              <p className="truncate text-sm text-muted-foreground">
                毎月{rule.monthly_transaction_date}日 ・ {rule.category_name} ・{' '}
            ...
            <div className="flex shrink-0 gap-1">
              <Button
                aria-label={`${rule.monthly_transaction_name}を編集`}
                onClick={() => onEdit(rule)}
                size="icon-sm"
                type="button"
                variant="ghost"
              ...
              <Button
                aria-label={
                  isPaused
                    ? `${rule.monthly_transaction_name}を再開`
                    : `${rule.monthly_transaction_name}を停止`
                }
                onClick={() => onSetIncluded(rule, isPaused)}
                size="icon-sm"
                type="button"
                variant="ghost"
              >
              ...
              <Button
                aria-label={`${rule.monthly_transaction_name}を完全に削除`}
