...
export function RecurringTransactionRuleEditor({
  ...
  const selectedCategory = categories.find(
    (category) => category.category_id === values.categoryId,
  )
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="space-y-2 sm:col-span-2">
          <label
            ...
            htmlFor="recurring-transaction-name"
          >
            取引名
          </label>
          <Input
            ...
            disabled={isSaving}
            id="recurring-transaction-name"
            maxLength={32}
            onChange={(event) => update('transactionName', event.target.value)}
            placeholder="例: 家賃"
        ...
        <div className="space-y-2">
          <label
            className="text-sm font-medium"
            htmlFor="recurring-transaction-day"
          >
            毎月の入力日
          </label>
          <div className="flex items-center gap-2">
        ...
        <div className="space-y-2">
          <label
            className="text-sm font-medium"
            htmlFor="recurring-transaction-category"
          >
            カテゴリ
          </label>
          <Select
          ...
          >
            <SelectTrigger
              ...
              id="recurring-transaction-category"
            >
        ...
        <div className="space-y-2">
          <label
            className="text-sm font-medium"
            htmlFor="recurring-transaction-subcategory"
          >
            サブカテゴリ
          </label>
          <Select
            disabled={isSaving || !values.categoryId}
            onValueChange={(value) => update('subcategoryId', value)}
            value={values.subcategoryId}
          >
            <SelectTrigger
              aria-invalid={errors.subcategoryId ? true : undefined}
              id="recurring-transaction-subcategory"
            >
        ...
        <div className="space-y-2 sm:col-span-2">
          <label
            className="text-sm font-medium"
            htmlFor="recurring-transaction-payment"
          >
            支払い方法
          </label>
          <Select
            disabled={isSaving}
            onValueChange={(value) =>
              update('paymentId', value === 'none' ? null : value)
            }
            value={values.paymentId ?? 'none'}
          >
            <SelectTrigger id="recurring-transaction-payment">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
