...
const chartPaletteOptions: ReadonlyArray<{
  ...
  { value: 'default', label: '標準', description: '現行のブルー基調' },
  {
    value: 'colorful',
    label: 'カラフル',
    description: '色相を分けて比較しやすい配色',
  },
  {
    value: 'monochrome',
    label: 'モノトーン',
    description: '濃淡で見分ける落ち着いた配色',
  },
]
...
function AppearancePanel({
  ...
  return (
    <Card aria-labelledby={titleId} className="block p-4 sm:p-5">
        ...
        <label key={option.value} className="group relative block">
          ...
          <span className="flex min-h-24 cursor-pointer flex-col items-center justify-center gap-1 rounded-xl border border-border bg-background px-1.5 py-2 text-center transition-colors peer-checked:border-foreground peer-checked:bg-muted peer-focus-visible:ring-3 peer-focus-visible:ring-ring/50 hover:bg-muted sm:min-h-28 sm:gap-2 sm:px-3 sm:py-3">
            ...
            <span className="min-w-0">
              <span className="block text-xs leading-4 font-semibold wrap-break-word sm:text-sm sm:leading-5">
                {option.label}
              </span>
              <span className="mt-0.5 block text-[0.6875rem] leading-4 wrap-break-word text-muted-foreground sm:mt-1 sm:text-xs sm:leading-5">
                {option.description}
              </span>
            </span>
            {accent === option.value ? (
      ...
      {chartPaletteOptions.map((option) => (
        <label key={option.value} className="group relative block">
          ...
          <span className="flex min-h-24 cursor-pointer flex-col items-center justify-center rounded-xl border border-border bg-background px-1.5 py-2 text-center transition-colors peer-checked:border-foreground peer-checked:bg-muted peer-focus-visible:ring-3 peer-focus-visible:ring-ring/50 hover:bg-muted sm:min-h-28 sm:px-3 sm:py-3">
            ...
            <span className="mt-2 block text-xs leading-4 font-semibold wrap-break-word sm:mt-3 sm:text-sm sm:leading-5">
              {option.label}
            </span>
            <span className="mt-0.5 text-[0.6875rem] leading-4 wrap-break-word text-muted-foreground sm:mt-1 sm:text-xs sm:leading-5">
              {option.description}
            </span>
...
export function AppearanceSettings() {
  return (
    <div className="space-y-5">
      <AppearancePanel
        description="アプリ全体の見た目を切り替えます。"
        title="テーマ"
      >
        <ThemePicker />
      </AppearancePanel>
      <AppearancePanel
        description="ボタンや選択状態など、主要なUIのアクセント色を選択できます。"
        title="アクセントカラー"
      >
        <AccentColorPicker />
      </AppearancePanel>
      <AppearancePanel
        description="ホームと分析画面のグラフで使用する色を選択できます。"
        title="グラフカラーセット"
      >
        <ChartPalettePicker />
      </AppearancePanel>
