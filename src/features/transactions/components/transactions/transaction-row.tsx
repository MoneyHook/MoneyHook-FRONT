...
function CategoryIcon({ item }: { item: TransactionItem }) {
  const presentation = getCategoryPresentation(item.categoryName, { isIncome: item.sign === 1 })
  const Icon = presentation.icon

  return (
    <span className={cn('flex size-10 shrink-0 items-center justify-center rounded-full sm:size-11', presentation.iconClassName)}>
      <Icon aria-hidden="true" className="size-5" />
    </span>
  )
}

export function TransactionRow({ item, onOpen }: { item: TransactionItem; onOpen: (id: string) => void }) {
  return (
    <button
    ...
    >
      ...
      <span className="min-w-0">
        <span className="block truncate text-sm font-semibold sm:text-base">{item.name}</span>
      ...
      <span className="flex min-w-0 items-center gap-2 sm:gap-4">
        ...
        <span className={cn('min-w-18 text-right text-base font-semibold tabular-nums sm:min-w-24 sm:text-lg', item.sign === -1 ? 'text-expense' : 'text-income')}>
