        ...
        {days.map((day) => {
          ...
          const dots = [
            ...new Map(
              dayItems.map((item) => [
                item.categoryName,
                getCategoryPresentation(item.categoryName, {
      ...
      {categories.length ? (
        <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-3">
          {categories.map((category) => {
            ...
            return (
              <div
                className="flex min-w-0 items-center gap-2 rounded-xl bg-muted/70 px-3 py-2"
                key={category.name}
              >
                <span
                  className={cn(
...
export function TransactionsCalendarPanel({
  data,
  month,
  onOpen,
  selectedDate,
