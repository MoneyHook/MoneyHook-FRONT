...
export function TransactionsView() {
  ...
  return (
    <>
          ...
          <div className="flex items-center gap-1">
            <div className="md:hidden">
              <TransactionFilterButton
                ...
                onClick={controller.openMobileFilters}
              />
            </div>
            <TransactionFilterButton
              activeCount={controller.activeFilterCount}
              className="hidden md:inline-flex"
              onClick={controller.openDesktopFilters}
            />
          </div>
        </header>
          ...
          >
            {controller.data && controller.activeFilterCount ? (
              <div className="space-y-3 pt-4 sm:pt-5">
                ...
                />
                <FilterResultSummary
                  sign={controller.filters.sign}
                  summary={controller.filterSummary}
                />
              </div>
            ) : null}
      ...
      <Sheet
        onOpenChange={controller.setMobileFilterOpen}
        open={controller.mobileFilterOpen}
      >
        <SheetContent
          className="max-h-[88svh] rounded-t-[2rem] border-x-0 px-0 pb-[max(1rem,env(safe-area-inset-bottom))]"
        ...
        >
          ...
          <div className="flex items-center justify-between px-5 pt-4 pb-1">
            ...
            <Button
              aria-label="絞り込みを閉じる"
              onClick={() => controller.setMobileFilterOpen(false)}
              size="icon-lg"
              variant="ghost"
            >
              <X aria-hidden="true" />
          ...
          <TransactionsFilterPanel
            draft={controller.draftFilters}
            onApply={controller.applyFilters}
            onChange={controller.setDraftFilters}
            onClear={() =>
              controller.setDraftFilters(EMPTY_TRANSACTION_FILTERS)
            }
            references={controller.references}
          />
        </SheetContent>
      ...
      <Sheet
        onOpenChange={controller.setDesktopFilterOpen}
        open={controller.desktopFilterOpen}
      >
        <SheetContent
          className="inset-y-4 right-4 hidden h-[calc(100%-2rem)] w-[min(26rem,calc(100vw-2rem))] max-w-none gap-0 rounded-l-[2rem] rounded-r-none border md:flex"
          showCloseButton={false}
          side="right"
        >
          <div className="flex items-center justify-between border-b px-6 py-5">
            ...
            <Button
              aria-label="絞り込みを閉じる"
              onClick={() => controller.setDesktopFilterOpen(false)}
              size="icon-lg"
              variant="ghost"
            >
              <X aria-hidden="true" />
      ...
      <Button
        aria-label="新しい取引を追加"
        className="fixed right-8 bottom-6 z-30 hidden size-14 rounded-full shadow-lg md:inline-flex"
        onClick={controller.openNewTransaction}
        size="icon-lg"
        title="新しい取引を追加"
      >
