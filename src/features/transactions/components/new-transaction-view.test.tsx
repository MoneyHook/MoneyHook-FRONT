...
const categories = [
  ...
  },
  {
    ...
    category_name: '収入',
    sub_category_list: [
      { sub_category_id: '21', sub_category_name: '給与', enable: true },
    ],
  },
]
...
describe('NewTransactionView', () => {
  ...
  it('renders persisted reference data before refreshing it from the API', async () => {
    ...
    expect(
      screen.queryByRole('status', { name: '取引追加画面を読み込んでいます' }),
    ...
    await waitFor(() => {
      expect(
        screen.getByRole('button', { name: 'ランチを候補から適用' }),
      ).toBeVisible()
      expect(
        localStorage.getItem(TRANSACTION_FORM_REFERENCE_CACHE_KEYS.categories),
      ).toContain('食費')
      expect(
        localStorage.getItem(TRANSACTION_FORM_REFERENCE_CACHE_KEYS.payments),
      ).toContain('楽天カード')
      expect(
        localStorage.getItem(
          TRANSACTION_FORM_REFERENCE_CACHE_KEYS.paymentTypes,
        ),
      ).toContain('カード')
      expect(
        localStorage.getItem(
  ...
  it('ignores the previous unlimited recommendation cache version', async () => {
    seedReferenceCache(
      ...
      {
        transaction_list: [
          { ...frequentTransaction, transaction_name: '旧キャッシュ候補' },
        ],
      },
    )
    ...
    expect(
      await screen.findByRole('status', {
        name: '取引追加画面を読み込んでいます',
      }),
    ).toBeVisible()
    expect(
      screen.queryByRole('button', { name: '旧キャッシュ候補を候補から適用' }),
    ).not.toBeInTheDocument()
  })
  ...
  it('shows validation errors for an incomplete form', async () => {
    ...
    expect(
      await screen.findByText('金額は1〜9,999,999円の整数で入力してください。'),
    ).toBeVisible()
    expect(
      screen.getByText('取引名は1〜32文字で入力してください。'),
    ).toBeVisible()
  ...
  it('applies a transaction candidate and saves the API-compatible payload', async () => {
    ...
    fireEvent.click(
      await screen.findByRole('button', { name: '取引候補をもっと表示' }),
    )
    const candidateSheet = await screen.findByRole('dialog')
    fireEvent.click(
      within(candidateSheet).getByRole('button', {
        name: 'ランチを候補から適用',
      }),
    )
  ...
  it('opens all transaction candidates in a bottom sheet', async () => {
    ...
    fireEvent.click(
      within(candidates).getByRole('button', { name: '取引候補をもっと表示' }),
    )
    const candidateSheet = await screen.findByRole('dialog')

    expect(
      within(candidateSheet).getByRole('heading', {
        name: 'おすすめをすべて表示',
      }),
    ).toBeVisible()
    expect(candidateSheet).toHaveClass('overflow-hidden')
    expect(candidateSheet).not.toHaveClass('overflow-y-auto')
    expect(candidateSheet.querySelector('.overflow-y-auto')).toHaveClass(
      'min-h-0',
      'flex-1',
    )
    expect(
      within(candidateSheet).getByText('すべての候補から選択できます'),
    ).toBeVisible()
    expect(
      within(candidateSheet).getAllByRole('button', {
        name: /候補\d+を候補から適用/,
      }),
    ).toHaveLength(13)
    ...
    await waitFor(() =>
      expect(screen.getByLabelText('取引名')).toHaveValue('候補13'),
    )
    expect(screen.queryByRole('dialog')).not.toBeInTheDocument()
  })
  ...
  it('does not show the more button when there are six or fewer transaction candidates', async () => {
    ...
    expect(
      within(candidates).queryByRole('button', {
        name: '取引候補をもっと表示',
      }),
    ).not.toBeInTheDocument()
  })
  ...
  it('shows only the category badge and transaction name in a candidate', async () => {
    ...
    const candidate = within(candidates).getByRole('button', {
      name: 'ランチを候補から適用',
    })
  ...
  it('uses semantic colors for the selected transaction sign', async () => {
    ...
    const transactionTypeTabs = screen.getByRole('tablist', {
      name: '取引区分',
    })
    ...
    expect(transactionTypeTabs).toHaveClass(
      'mt-3',
      'p-0.5',
      'sm:mt-8',
      'sm:p-1.5',
    ...
    expect(candidateButton.querySelector('[data-slot="badge"]')).toHaveClass(
      'h-9',
      'px-3',
      'text-sm',
    )
    expect(
      candidateButton.compareDocumentPosition(saveButton) &
  ...
  it('does not show the payment method section when no payment methods exist', async () => {
    ...
    expect(
      screen.queryByRole('button', { name: /支払い方法/ }),
    ).not.toBeInTheDocument()
  })
  ...
  it('selects the locally configured default payment method for a new transaction', async () => {
    ...
    expect(
      await screen.findByRole('button', { name: '支払い方法楽天カード' }),
    ).toBeVisible()
  })
  ...
  it('clears a default payment method that no longer exists', async () => {
    ...
    expect(
      await screen.findByRole('button', { name: '支払い方法選択しない' }),
    ).toBeVisible()
    await waitFor(() =>
      expect(
        localStorage.getItem('moneyhooks:user-cache:default-payment'),
      ).toBeNull(),
    )
  })
  ...
  it('selects a category and subcategory in the same sheet', async () => {
    ...
    expect(
      await screen.findByRole('heading', { name: 'サブカテゴリを選択' }),
    ).toBeVisible()
    expect(
      screen.getByText('食費のサブカテゴリを選択してください。'),
    ).toBeVisible()

    fireEvent.click(screen.getByRole('button', { name: 'カテゴリ選択へ戻る' }))
    expect(
      await screen.findByText('取引のカテゴリを選択してください。'),
    ).toBeVisible()
    fireEvent.click(screen.getByRole('button', { name: '収入' }))
    expect(
      await screen.findByText('収入のサブカテゴリを選択してください。'),
    ).toBeVisible()

    fireEvent.click(screen.getByRole('button', { name: '給与' }))
    await waitFor(() =>
      expect(
        screen.getByRole('button', { name: /カテゴリ.*収入.*給与/ }),
      ).toBeVisible(),
    )
  })
