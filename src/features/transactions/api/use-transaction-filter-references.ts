...
export function useTransactionFilterReferences(enabled: boolean) {
  const categoriesQuery = useGetCategoryWithSubCategoryList({ query: { enabled } })
  const paymentsQuery = useGetPaymentResources({ query: { enabled } })

  return {
    categories: categoriesQuery.data?.status === 200
      ? (categoriesQuery.data.data.category_list ?? [])
      : [],
    payments: paymentsQuery.data?.status === 200 ? paymentsQuery.data.data.payment_list : [],
    isPending: enabled && (categoriesQuery.isPending || paymentsQuery.isPending),
