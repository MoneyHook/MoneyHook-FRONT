...
function isCategoryResponse(
  value: unknown,
): value is CategoryWithSubcategoryResponse {
  return (
    isRecord(value) &&
    (value.category_list === null || Array.isArray(value.category_list))
  )
}

function isPaymentResourcesResponse(
  value: unknown,
): value is PaymentResourceListResponse {
  return hasArray(value, 'payment_list')
}

function isPaymentTypesResponse(
  value: unknown,
): value is PaymentTypeListResponse {
  return hasArray(value, 'payment_type_list')
}

function isFrequentTransactionsResponse(
  value: unknown,
): value is FrequentTransactionResponse {
...
export function useTransactionFormReferences({ isEdit }: { isEdit: boolean }) {
  ...
  const [cachedPayments] = useState(() =>
    readPersistedUserData(
      TRANSACTION_FORM_REFERENCE_CACHE_KEYS.payments,
      CACHE_VERSION,
      isPaymentResourcesResponse,
    ),
  )
